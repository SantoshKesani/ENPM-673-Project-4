...
def tracker(img, tmp, rect, p, count):
    ...
    while threshold > 0.05:
        ...
        grad_x = cv2.Sobel(np.float64(new_img), cv2.CV_64F, 1, 0, ksize=5)
        grad_y = cv2.Sobel(np.float64(new_img), cv2.CV_64F, 0, 1, ksize=5)
...
for img in frames_list:
    ...
    p = tracker(gray_img, Tmp_img, rect, p, count)
